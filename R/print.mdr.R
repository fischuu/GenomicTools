  ...
  for (i in 1:tempFold) {
    ...
    if (i == 1) {
      ...
      if (length(x$res.sampled) > 0) {
        ...
        for (cvcRun in 1:length(x$res.sampled)) {
          ...
          temp2.cvc[[cvcRun]] <-
            colnames(x$X)[unlist(x$res.sampled[precPos[i] + 1])[1:x$top]]
          if (fix != -1) {
            temp2 <- temp2[1]
          }
        }
      ...
      temp2 <-
        paste(colnames(x$X)[tempPos1], colnames(x$X)[tempPos2], sep = ",")
      if (length(x$res.sampled) > 0) {
        temp2.cvc <- list()
        for (cvcRun in 1:length(x$res.sampled)) {
          tempPos1 <- unlist(x$res.sampled[[cvcRun]][precPos[i] + 1])[1:x$top]
          tempPos2 <-
            unlist(x$res.sampled[[cvcRun]][precPos[i] + 2])[1:x$top]
          temp2.cvc[[cvcRun]] <-
            paste(colnames(x$X)[tempPos1], colnames(x$X)[tempPos2], sep = ",")
        }
      }
      ...
      temp2 <-
        paste(colnames(x$X)[tempPos1], colnames(x$X)[tempPos2], colnames(x$X)[tempPos3], sep =
            ",")
      if (length(x$res.sampled) > 0) {
        temp2.cvc <- list()
        for (cvcRun in 1:length(x$res.sampled)) {
          ...
          tempPos2 <-
            unlist(x$res.sampled[[cvcRun]][precPos[i] + 2])[1:x$top]
          tempPos3 <-
            unlist(x$res.sampled[[cvcRun]][precPos[i] + 3])[1:x$top]
          temp2.cvc[[cvcRun]] <-
            paste(colnames(x$X)[tempPos1],
              colnames(x$X)[tempPos2],
              colnames(x$X)[tempPos3],
              sep = ",")
        }
      }
    }
      ...
      if (length(x$res.sampled) > 0) {
        ...
        for (cvcRun in 1:length(x$res.sampled)) {
          ...
          tempPos3 <-
            unlist(x$res.sampled[[cvcRun]][precPos[i] + 3])[1:x$top]
          tempPos4 <-
            unlist(x$res.sampled[[cvcRun]][precPos[i] + 4])[1:x$top]
          temp2.cvc[[cvcRun]] <-
            paste(colnames(x$X)[tempPos1],
              colnames(x$X)[tempPos2],
              colnames(x$X)[tempPos3],
    ...
    if (length(x$res.sampled) > 0) {
      ...
      for (resRun in 1:length(temp2)) {
        for (cvcRun in 1:length(x$res.sampled)) {
          cvc[resRun] <-
            cvc[resRun] + sum(is.element(temp2[resRun], temp2.cvc[[cvcRun]]))
        }
      }
      cvc <- cvc / length(x$res.sampled)
      tempDF <- data.frame(SNP = temp2,
        Acc = temp,
        CVC = cvc)
    }
    else {
