summary.eqtl <- function(object, sig = 0.01, ...) {
  ...
  cat("Total number of SNPs (in geno object)         :",
      dim(object$geno[[3]])[1],
      "\n")
  cat("Window size (in MB)                           :", ws, "\n")
  if (!trans) {
    cat(
      "Average (median) number of SNP in window      :",
      mean(sapply(p.values, length)),
      "(",
      median(sapply(p.values, length)),
      ")\n"
    )
  }
  sigP <- sapply(p.values, function(x) {
    sum(x <= sig, na.rm = TRUE)
  })
  if (!trans) {
    cat(
      "Average (median) number of sig. (p<",
      sig,
      ") SNP in window :",
      mean(sigP),
      "(",
