eqtlDir <- function(genoGroups, gex, mc = mc, nper, testType, MAF) {
  innerFunction <- function(i) {
    # Calculate MAF
    maf.snp <-
      sum(genoGroups[genoGroups[, i] < 3, i]) / (2 * (sum(genoGroups[, i] < 3)))
    ...
    if (sum(genoGroups[, i] == 3) < (nrow(genoGroups))) {
      ...
      if (PI == 1) {
        ...
        if (testType == "asymptotic") {
          testType <- "external"
        }
        innerOut <-
          gmw(
            gex[genoGroups[, i] != 3],
            genoGroups[genoGroups[, i] != 3, i],
            test = "mw",
            type = testType,
        ...
        p2 <-
          gmw(
            gex[genoGroups[, i] != 3],
            createGroups(genoGroups[genoGroups[, i] != 3, i], c(2, 1, 0)),
            test = "triple",
            type = testType,
            alternative = "greater",
            nper = nper
  ...
  if (is.matrix(genoGroups)) {
    output <-
      unlist(mclapply(1:ncol(genoGroups), innerFunction, mc.cores = mc))
  }
  else {
    genoGroups <- as.matrix(genoGroups)
    if (ncol(genoGroups) > 1) {
      genoGroups <- t(genoGroups)
    }
