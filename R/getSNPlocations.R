getSNPlocations <- function(genotInfo, annot, th) {
  ...
  if (!is.null(th)) {
    ...
    if (is.data.table(genotInfo)) {
      ...
      th <- th * 10 ^ 6
      chrSNPs <-
        genotInfo[genotInfo$V1 == as.character(annot[1, 1]), ]
      lowSNPs <- chrSNPs[chrSNPs$V4 > (annot$Start - th), ]
      SNPs <- lowSNPs[lowSNPs$V4 < (annot$End + th), ]
      #    if(is.null(th)) SNPs <- chrSNPs
      output <-
        list(SNPloc = as.data.frame(SNPs), SNPcol = as.numeric(rownames(SNPs)))
      ...
      chrSNPs <-
        genotInfo[genotInfo[, 1] == as.character(annot[1, 1]), ]
      ...
      #    if(is.null(th)) SNPs <- chrSNPs
      output <-
        list(SNPloc = SNPs, SNPcol = as.numeric(rownames(SNPs)))
    }
  }
  else {
