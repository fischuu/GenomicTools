...
QTL <-
  function(pheno,
    ...
    if (is.null(pheno)) {
      stop("No phenotype information provided in pheno!")
    }
    if (is.null(geno)) {
    ...
    if (is.character(geno) == TRUE && length(geno) == 1)
    {
      ...
      if (class(geno) == "PedMap") {
        ...
        if (length(geno) != nrow(pheno)) {
          stop("Amount of entered phenotypes and genotypes do not match!")
        }
    ...
    if (noNames) {
      if (is.null(phenoSamples)) {
        if (verbose) {
          cat(
            "Phenotype are unnamed. We assume same order in phenotype and genotype objects and match samples based on that.\n"
          )
        }
    ...
    if (sum(overlap) == 0) {
      stop("No matching phenotype / genotype sample names!\n")
    }
    if (verbose == TRUE) {
      cat(
        "We have for",
        olPerc,
        "% of the samples in the phenotype data the genotype information available. \n"
      )
    }
    if (verbose == TRUE) {
      cat(
        "We have for",
        olPerc2,
        "% of the samples in the genotype data the phenotype values available. \n"
      )
    }
    
    # Gene statistics
    if (is.null(colnames(pheno))) {
      colnames(pheno) <- paste("Phenotype", 1:ncol(pheno), sep = "")
    }
    matchingPheno <- colnames(pheno)
    if (verbose == TRUE) {
      cat("We will test for",
        ncol(pheno),
        "phenotypes possible QTLs! \n")
    }
    if (verbose == TRUE) {
      cat("---------------------------------------------- \n")
    }
    ...
    pheno <-
      pheno[is.element(rownames(pheno), genoSamples), , drop = FALSE]
    ...
    for (phenoRun in 1:length(matchingPheno)) {
      ...
      if (dim(SNPloc$SNPloc)[1] > 0) {
        ...
        genoGroups <-
          rearrange(genoGroups, rownames(pheno), genoSamples)
        ...
        if (method == "LM") {
          ...
          if (is.null(sig)) {
            if (is.matrix(genoGroups)) {
              qtlTemp[[phenoRun]] <- list(
                GeneLoc = rep(phenoRun, ncol(genoGroups)),
                TestedSNP = SNPloc[[1]],
                p.values = eqtlDir(genoGroups, pheno[, phenoRun], mc, nper, testType)
              )
            }
            else {
              qtlTemp[[phenoRun]] <- list(
            ...
            p.values <-
              eqtlDir(genoGroups, pheno[, phenoRun], mc, nper, testType)
            pPos <- p.values <= sig
            qtlTemp[[phenoRun]] <-
              cbind(SNPloc[[1]][pPos, c(1, 2, 4)], p.values[pPos])
          }
        ...
        if (sum(p.values <= sig) == 0) {
          qtl[[phenoRun]] <-
            data.frame(
              ...
              Location = "-1",
              p.value = "-1",
              Assoc.Pheno = matchingPheno[phenoRun],
          ...
          bedTemp <-
            cbind(bedTemp, rep(matchingPheno[phenoRun], max(1, nrow(bedTemp))))
          colnames(bedTemp) <-
            c("chr", "SNP", "Location", "p.value", "Assoc.Pheno")
          qtl[[phenoRun]] <- bedTemp
        }
      }
      if (verbose == TRUE) {
        cat (
          "We calculated QTLs for ",
          matchingPheno[phenoRun],
          " for ",
