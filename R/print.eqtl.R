  ...
  if (x$type == "full") {
    ...
    if (output == "bed") {
        ...
        if (nrow(repSNP) > 0) {
          temp <-
            data.frame(
              ...
              End = repSNP[, 4],
              Name = repSNP[, 2],
              Gene = names(xx)[testRun],
              p.value = tempRes[[3]][tempRes[[3]] <= sig]
            )
      ...
      for (gene in 1:length(xx)) {
        ...
        for (sub in 1:Nlocs) {
          temp <-
            xx[[gene]]$TestedSNP[((xx[[gene]]$p.values > -1) &
                (xx[[gene]]$p.values <= sig)), c(1, 2, 4, 5, 6)]
          temp2 <-
            xx[[gene]]$p.values[((xx[[gene]]$p.values > -1) &
                (xx[[gene]]$p.values <= sig))]
