genotypePlot <-
  function(snp,
    ...
    if (!is.vector(eqtl$gex)) {
      ...
      if (sum(colnames(eqtl$gex) == gene) == 0) {
        stop("There are no expression values provided for the gene", gene)
      }
    }
    ...
    ..snpCol <-
      NULL  # This is just for the Cran check, hopefully it does not do any harm...
    snpValues <-
      as.numeric(as.vector(as.matrix(eqtl$geno$genotypes[, ..snpCol])))
    ...
    for (i in 1:length(nGroups)) {
      ...
      if (gene == "") {
        ...
        grExpr[[i]] <-
          eqtl$gex[is.element(rowGex, temp), colnames(eqtl$gex) == gene]
      }
    ...
    ifelse(
      mainlab == TRUE,
      mainTitle <-
        paste(snp, " and ", gene, " (P-value:", snpP, ")", sep = ""),
      mainTitle <- ""
    )
    
    boxplot(
      grExpr[[1]],
      xlim = c(0.5, length(grExpr) + 0.5),
      ylim = c(min(as.vector(eqtl$gex)), max(as.vector(eqtl$gex))),
      main = mainTitle,
      ylab = ylab,
      xlab = xlab
    )
