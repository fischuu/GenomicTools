plot.trans <- function(x, genome) {
  ...
  for (geneRun in 1:length(geneAnnot)) {
    tmpAssoc <-
      x$bed[as.character(x$bed$Assoc.Gene) == names(geneAnnot)[geneRun],]
    ...
    for (assocRun in 1:nrow(tmpAssoc)) {
      if (is.element(tmpAssoc$chr[assocRun], genome$chr)) {
        ...
        circos.link(
          sector.index1 = geneChr,
          point1 = c(tmpGene$Start / 10 ^ 6, tmpGene$End / 10 ^ 6),
          sector.index2 = snpChr,
          point2 = c(tmpAssoc$Location[assocRun] / 10 ^ 6),
          col = lcols[geneChr]
        )
      }
      else {
  ...
  if (wrongCHR) {
    warning(
      "There were sig. SNPs that coulnd't be matched to the genome information! Maybe wrong genome, or mislabeled chromosomes?"
    )
  }
