eQTL <-
  function(gex = NULL,
    ...
    if (sum(class(xAnnot) == "gtf") > 0) {
      if (verbose) {
        cat(
          "xAnnot is given in gtf format (from importGTF). We transform it with gtfToBed() into required bed-format.\n"
        )
      }
    ...
    if (is.null(windowSize) & is.null(sig)) {
      if (!IHaveSpace) {
        ...
        if (verbose) {
          cat("You set the IHaveSpace=TRUE option to get all results for the trans-eQTL run!\n")
        }
      }
    }
    ...
    if (is.null(gex)) {
      stop("No gene expression provided in gex!")
    }
    if (is.null(geno)) {
      stop("No genotypes provided in geno!")
    }
    if (is.vector(gex)) {
      tmp <- names(gex)
      oldNames <- names(gex)
    ...
    if (is.data.frame(xAnnot)) {
      if (is.factor(xAnnot[, 1])) {
        xAnnot[, 1] <- as.character(xAnnot[, 1])
      }
      if (is.factor(xAnnot[, 4])) {
        xAnnot[, 4] <- as.character(xAnnot[, 4])
      }
      if (verbose == TRUE) {
        cat("We will transform the gene annotations into a list ... ", sep = "")
      }
    ...
    if (is.character(geno) == TRUE) {
      ...
      fileEnding <-
        tolower(substr(geno, nchar(geno) - 2, nchar(geno)))
      if (fileEnding == "ped") {
        case <- "ped"
        pedFile <- geno
        mapFile <-
          paste(substr(geno, 1, nchar(pedFile) - 3), "map", sep = "")
      }
        ...
        if (verbose) {
          cat(
            "No .ped or .vcf file ending detected in geno. Assume geno to be a ped/map filepair!\n"
          )
        }
        case <- "ped"
      ...
      if (case == "vcf") {
        ...
        if (verbose == TRUE) {
          cat("ped-file:", pedFile, "\n")
        }
        if (verbose == TRUE) {
          cat("map-file:", mapFile, "\n")
        }
        genoData <-
          importPED(file = pedFile,
            snps = mapFile,
            verbose = FALSE)
      }
      ...
      if (class(geno) == "PedMap") {
        ...
        stop(
          "Please provide either a PedMap (importPED) of a VCF (importVCF) object, or the corresponding file path to either file."
        )
      }
    }
    ...
    if ((ncol(gex) == 1) & is.null(xAnnot)) {
      ...
      xAnnot <-
        data.frame(
          Chr = as.character(genoData$map[, 1]),
          Start = genoData$map[, 4],
          End = genoData$map[, 4],
          Gene = as.character(genoData$map[, 2])
        )
      xAnnot <- makeAnnotList(xAnnot)
      windowSize = NULL
    }
    if ((ncol(gex) > 1) & is.null(xAnnot)) {
      warning(
        "No annotations given, we will test all given SNPs against all given expressions!\n gex is a matrix, hence this might take a while...\n"
      )
      xAnnot <-
        data.frame(
          Chr = as.character(genoData$map[1, 1]),
          Start = genoData$map[1, 4],
          End = genoData$map[1, 4],
          Gene = ""
        )
      #    xAnnot <- makeAnnotList(xAnnot)
      windowSize = NULL
    ...
    if (is.null(genoSamples)) {
      genoSamples <-  rownames(genoData$genotypes)
    }
    ...
    olPerc2 <-
      round(sum(overlap) / nrow(genoData$genotypes) * 100, 1)
    if (sum(overlap) == 0) {
      stop("No matching expression / genotype sample names!\n")
    }
    if (verbose == TRUE) {
      cat(
        "We have for",
        olPerc,
        "% of the samples in the expression data the genotype information available. \n"
      )
    }
    if (verbose == TRUE) {
      cat(
        "We have for",
        olPerc2,
        "% of the samples in the genotype data the expression values available. \n"
      )
    }
    ...
    if (sum(overlap) == 0) {
      stop("No matching expression names / gene annotations!\n")
    }
    if (verbose == TRUE) {
      cat("We have for",
        olPerc,
        "% of the expression data the annotations. \n")
    }
    
    # Gene statistics
    matchingGenes <-
      colnames(gex)[is.element(colnames(gex), names(xAnnot))]
    if (verbose == TRUE) {
      cat("We will test for",
        length(matchingGenes),
        "genes possible eQTLs! \n")
    }
    if (verbose == TRUE) {
      cat("---------------------------------------------- \n")
    }
    ...
    gex <-
      gex[is.element(rownames(gex), genoSamples), , drop = FALSE]
    ...
    for (geneRun in 1:length(matchingGenes)) {
      ...
      tempAnnot <-
        xAnnot[[which((names(xAnnot) == matchingGenes[geneRun]) == TRUE)]]
      eqtlTemp <- list()
      
      for (tempRun in 1:nrow(tempAnnot)) {
        # SNP locations of variants inside the provided window
        SNPloc <-
          getSNPlocations(genotInfo = genoData$map,
            annot = tempAnnot[tempRun, ],
            th = windowSize)
        
        # Run eQTL only if there are SNPs inside the window
        if (dim(SNPloc$SNPloc)[1] > 0) {
          SNPmatrix <-
            as.matrix(genoData$genotypes[, SNPloc$SNPloc$snp.names, with = FALSE])
          genoGroups <-
            matrix(as.numeric(SNPmatrix), nrow = nrow(SNPmatrix))
          colnames(genoGroups) <- colnames(SNPmatrix)
          rownames(genoGroups) <- rownames(genoData)
          genoGroups <-
            rearrange(genoGroups, rownames(gex), genoSamples)
          ...
          if (method == "LM") {
            ...
            if (is.null(sig)) {
              ...
              p.values <- eqtlLM(genoGroups, gex[, geneRun], MAF, mc)
              pPos <- p.values <= sig
              eqtlTemp[[tempRun]] <-
            ...
            if (is.null(sig)) {
              if (is.matrix(genoGroups)) {
                eqtlTemp[[tempRun]] <- list(
                  GeneLoc = rep(tempRun, ncol(genoGroups)),
                  TestedSNP = SNPloc[[1]],
                  p.values = eqtlDir(genoGroups, gex[, geneRun], mc, nper, testType, MAF)
                )
              }
              else {
                eqtlTemp[[tempRun]] <- list(
              ...
              p.values <-
                eqtlDir(genoGroups, gex[, geneRun], mc, nper, testType, MAF)
              pPos <- p.values <= sig
              eqtlTemp[[tempRun]] <-
                cbind(SNPloc[[1]][pPos, c(1, 2, 4)], p.values[pPos])
            }
          ...
          warning("There were no variants within the window of gene ",
            matchingGenes[geneRun])
          p.values <-
            2  # Take a p-value of 2 as internal indicator that there wasn't anything to test!
          eqtlTemp[[tempRun]] <-
            data.frame(
              GeneLoc = -1,
              TestedSNP = -1,
        ...
        if (sum(p.values <= sig, na.rm = TRUE) == 0) {
          eqtl[[geneRun]] <-
            data.frame(
              ...
              Location = "-1",
              p.value = "-1",
              Assoc.Gene = matchingGenes[geneRun],
          ...
          bedTemp <-
            cbind(bedTemp, rep(matchingGenes[geneRun], max(1, nrow(bedTemp))))
          colnames(bedTemp) <-
            c("chr", "SNP", "Location", "p.value", "Assoc.Gene")
      ...
      if (verbose == TRUE) {
        ...
        avgRuntime <-
          as.numeric(difftime(now, eqtlStart, units = "secs")) / geneRun
        reqTime <-
          sectoDay((length(matchingGenes) - geneRun) * avgRuntime)
        
        cat (
          "We calculated eQTLs for ",
          matchingGenes[geneRun],
          " for ",
          prettyNum(nrow(SNPloc$SNPloc), big.mark = ","),
          " SNPs - ",
          date(),
          ", remaing time: ",
          reqTime,
          " for ",
          length(matchingGenes) - geneRun,
          "/",
          length(matchingGenes),
          " genes\n",
          sep = ""
    ...
    if (is.null(sig)) {
      ...
      if (length(remThese) > 0) {
        resBed <- resBed[-remThese, ]
      }
      result <-
        list(
          bed = resBed,
