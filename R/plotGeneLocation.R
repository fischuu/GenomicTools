plotGeneLocation <-
  function(bed,
    ...
    if (is.null(xlim)) {
      ...
      if (is.null(chrInfo)) {
        xlim <-
          c(min(as.numeric(bed[, 2])) / 10 ^ 6 - 0.5, max(as.numeric(bed[, 3])) /
              10 ^ 6 + 0.5)
      }
      else {
    ...
    if (is.null(order)) {
      geneLocations <-
        mclapply(tempGenes,
          createGeneBoundaries,
          bed = bed,
          mc.cores = mc.cores)
      bind.ith.rows <-
        function(i)
          do.call(rbind, lapply(geneLocations, "[", i, TRUE))
    ...
    if (is.null(order)) {
      ...
      checkLayer <- function(pos, start) {
        for (i in start:nrow(pos)) {
          ...
          if (((pos[i, 1] - pos[start - 1, 2]) < minDistance) &&
      ...
      for (i in 1:length(order)) {
        ...
        layerPos <-
          which((is.element(
            as.character(geneLocationsJoined[, 3]), order[i]
          ) == TRUE))
