...
`plotSingle.eqtl` <-
  function(x,
    ...
    if (x$type == "full") {
      ...
      if (verbose == TRUE) {
        cat(
          "The plot function was applied to the 'sig' eQTL - this means the eQTL has to be performed again for each single plot. \n"
        )
        cat(
          "The 'full' stores all required information to plot it directly, but consumes more memory) \n"
        )
      }
      ...
      for (i in 1:length(which)) {
        if (sum(is.element(colnames(x$gex), which[i])) > 0 &
            sum(is.element(names(x$xAnnot), which[i])) > 0) {
          tempEQTL <-
            eQTL(
              gex = x$gex,
              geno = x$geno,
          ...
          warning(
            "Check your 'which' parameter, there are no expression and/or annotation for the object ",
            which[i],
            "\n"
          )
        }
...
plotIt <- function(x,
  ...
  for (gene in 1:length(x)) {
    ...
    if (Nlocs == 0) {
      ...
      maxX <-
        as.numeric(temp$GeneInfo[1,][3] + windowSize * 10 ^ 6)
      ...
      if (double == FALSE) {
        plot(
          ...
          ylim = plotY,
          xlim = c(0, 20),
          xlab = "Chromosomal Position in MB",
          ylab = plotYlab,
          main = paste(names(x)[gene], "- NO SNPs found"),
          sub = paste(
            "Chr",
            temp$GeneInfo[1, 1],
            ":",
            prettyNum(minX, big.mark = ","),
            "-",
            prettyNum(maxX, big.mark = ",")
          ),
          yaxt = "n",
          xaxt = "n"
        )
        axis(1,
          at = seq(0, 20, 4),
          labels = seq(round(minX / 10 ^ 6, 1), round(maxX / 10 ^ 6, 1), length.out =
              6))
      }
      else {
        plot(
      ...
      axis(
        2,
        at = c(seq(0, 1, 0.2), 1.1, 1.2, 1.3),
        labels = c(seq(0, 1, 0.2), "MAF" , "Mono.", "NA")
      )
      ...
      for (sub in 1:Nlocs) {
        ...
        if (centered == TRUE) {
          ...
          maxX <-
            as.numeric(temp$GeneInfo[sub,][3] + windowSize * 10 ^ 6)
        }
        else {
        ...
        xPos <-
          (temp$TestedSNP[subPos, 4] - minX) / (windowSize * 10 ^ 5)
        yPos <- temp$p.values[subPos]
        if (!is.null(x2)) {
          yPos2 <- temp2$p.values[subPos]
        }
        ...
        if (log == FALSE) {
          ...
          for (i in 1:length(xPos)) {
            if (is.null(x2)) {
              ...
              if (col[i] == "red") {
                points(xPos[i], yPos[i], col = "red", pch = 20)
              }
            }
            else {
              if (yPos[i] > yPos2[i]) {
                ...
                lines(c(xPos[i], xPos[i]), c(0, yPos2[i]), col =
                    col2[i])
                if (col2[i] == "steelblue") {
                  points(xPos[i],
                    yPos2[i],
        ...
        if (!is.null(annot)) {
          ...
          plot(
            ...
            ylab = "",
            main = "",
            sub = paste("Chr", temp$GeneInfo[sub, 1], ":", minX, "-", maxX),
            xaxt = "n"
          )
          axis(
            1,
          ...
          for (annotRun in 1:nrow(annot)) {
            lines(c(annot[annotRun, 3] / 10 ^ 6, annot[annotRun, 4] / 10 ^ 6), c(0.5, 0.5), lwd =
                2)
