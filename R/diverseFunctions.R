...
vectorToGenomatrix <- function(x) {
  ...
  if (grep("/", x[1]) == 1) {
    ...
    x.factor <-
      factor(as.character(x), levels = c(
...
matrixToGenomatrix <- function(x) {
  ...
  for (i in 1:ncol(x)) {
    if (grep("/", x[1, i]) == 1) {
      ...
      levels(x.factor)[levels(x.factor) == paste(alleles[1], "/", alleles[1], sep =
          "")] <- "01"
      levels(x.factor)[levels(x.factor) == paste(alleles[1], "/", alleles[2], sep =
          "")] <- "02"
      levels(x.factor)[levels(x.factor) == paste(alleles[2], "/", alleles[1], sep =
          "")] <- "02"
      levels(x.factor)[levels(x.factor) == paste(alleles[2], "/", alleles[2], sep =
          "")] <- "03"
      if (i == 1) {
        out$genotypes <- as.matrix(x.factor)
      }
