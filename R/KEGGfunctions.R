...
getKEGGOrganisms <-
  function(url = "http://rest.kegg.jp/list/organism") {
    tmp <- readLines(url)
    tmp <- strsplit(tmp, "\t")
    out <- data.frame(
      KEGG = sapply(tmp, "[", 1),
...
getKEGGOrthologOverview <- function () {
  ...
  tmp <- strsplit(tmp, "\t")
  out <-
    data.frame(ortholog = sapply(tmp, "[", 1),
      description = sapply(tmp,
        "[", 2))
  out
}
...
getKEGGPathwayOverview <- function(code = "hsa") {
  tmp <-
    readLines(paste("http://rest.kegg.jp/list/pathway/", code, sep = ""))
  tmp <- strsplit(tmp, "\t")
...
getKEGGInformation <- function(x) {
  ...
  for (i in 1:length(tmp)) {
    ...
    if (substr(tmp[i], 1, 1) != " ") {
      ...
      tmpEntry <- data.frame(
        Entry = tmpLine[3],
        Description = paste(tmpLine[4:length(tmpLine)], collapse =
            " "),
      ...
      tmpLine <- unique(strsplit(tmp[i], " ")[[1]])
      # Create a temporary matrix that stores the information for that particular field
      tmpEntry2 <- data.frame(
        Entry = tmpLine[2],
        Description = paste(tmpLine[3:length(tmpLine)], collapse =
            " "),
        stringsAsFactors = FALSE
      )
...
getKEGGModule <- function(module) {
  tmp <-
    readLines(paste("http://rest.kegg.jp/get/", module, sep = ""))
  ...
  if (length(ORTHOLOGY) > 0) {
    ...
    ORTHOLOGY.tmp[1] <-
      gsub("^ORTHOLOGY", "         ", ORTHOLOGY.tmp[1])
    ORTHOLOGY.tmp <- trimws(ORTHOLOGY.tmp)
    
    ORTHOLOGY.tmp <- strsplit(ORTHOLOGY.tmp, " ")
    
    ORTHOLOGY.out <-
      data.frame(Orthology = sapply(ORTHOLOGY.tmp, "[", 1),
        Desc = trimws(sapply(
          sapply(ORTHOLOGY.tmp, "[",-1), paste, collapse = " "
        )))
...
getKEGGPathway <- function(pathway) {
  tmp <-
    readLines(paste("http://rest.kegg.jp/get/", pathway, sep = ""))
  ENTRY <- tmp[which(grepl("^ENTRY", tmp) == TRUE)]
  NAME <- tmp[which(grepl("^NAME", tmp) == TRUE)]
...
getKEGGPathwayImage <- function(pathway, folder = NULL) {
  ...
  dlURL <-
    paste("http://rest.kegg.jp/get/", pathway, "/image", sep = "")
  download.file(
    url = dlURL,
